...
namespace WixToolset.Netfx
{
    ...
    public sealed class NetfxCompiler : BaseCompilerExtension
    {
                ...
                case "Fragment":
                    switch (element.Name.LocalName)
                    {
                        ...
                        case "DotNetCompatibilityCheckRef":
                            ...
                            break;
                        case "DotNetSdkCompatibilityCheck":
                            this.ParseDotNetSdkCompatibilityCheckElement(intermediate, section, element);
                            break;
                        case "DotNetSdkCompatibilityCheckRef":
                            this.ParseDotNetSdkCompatibilityCheckRefElement(intermediate, section, element);
                            break;
                        default:
                            this.ParseHelper.UnexpectedElement(parentElement, element);
                            break;
        ...
        /// <summary>
        /// Parses a DotNetSdkCompatibilityCheck element.
        /// </summary>
        /// <param name="element">The element to parse.</param>
        private void ParseDotNetSdkCompatibilityCheckElement(Intermediate intermediate, IntermediateSection section, XElement element)
        {
            var sourceLineNumbers = this.ParseHelper.GetSourceLineNumbers(element);
            Identifier id = null;
            string property = null;
            ...
            if (null == id)
            {
                id = this.ParseHelper.CreateIdentifier("ndnscc", property,  platform, version);
            }
            ...
            this.ParseHelper.CreateCustomActionReference(sourceLineNumbers, section, "Wix4NetFxDotNetSdkCompatibilityCheck", this.Context.Platform, CustomActionPlatforms.ARM64 | CustomActionPlatforms.X64 | CustomActionPlatforms.X86);

            if (!this.Messaging.EncounteredError)
            {
                section.AddSymbol(new NetFxDotNetSdkCompatibilityCheckSymbol(sourceLineNumbers, id)
                {
                    Platform = platform,
                    Version = version,
                    RollForward = rollForward,
        ...
        /// <summary>
        /// Parses a DotNetSdkCompatibilityCheckRef element.
        /// </summary>
        /// <param name="element">The element to parse.</param>
        private void ParseDotNetSdkCompatibilityCheckRefElement(Intermediate intermediate, IntermediateSection section, XElement element)
        {
            var sourceLineNumbers = this.ParseHelper.GetSourceLineNumbers(element);
            ...
            foreach (var attrib in element.Attributes())
            {
                if (String.IsNullOrEmpty(attrib.Name.NamespaceName) || this.Namespace == attrib.Name.Namespace)
                {
                    switch (attrib.Name.LocalName)
                    {
                        case "Id":
                            var refId = this.ParseHelper.GetAttributeIdentifierValue(sourceLineNumbers, attrib);
                            this.ParseHelper.CreateSimpleReference(section, sourceLineNumbers, NetfxSymbolDefinitions.NetFxDotNetSdkCompatibilityCheck, refId);
                            break;
                        default:
                            this.ParseHelper.UnexpectedAttribute(element, attrib);
                            break;
